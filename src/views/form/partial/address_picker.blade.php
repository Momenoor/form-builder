...
<script type="text/javascript"
    src='https://maps.google.com/maps/api/js?sensor=false&libraries=places&key={{ config('form-builder.api_google_map_key') }}'>
</script>
<script>
        ...
        jQuery('#{{ $uniqId }}-map').locationpicker({
            ...
            @if (empty($noEdit))
                ...
                onchanged: function(currentLocation, radius, isMarkerDropped) {
                    ...
                    .addressComponents;
                    jQuery('input[name="{{ $name . '[street]' }}"]').val(addressComponents
                        .addressLine1);
                    jQuery('input[name="{{ $name . '[city]' }}"]').val(addressComponents.city);
                    jQuery('input[name="{{ $name . '[state]' }}"]').val(addressComponents
                        .stateOrProvince);
